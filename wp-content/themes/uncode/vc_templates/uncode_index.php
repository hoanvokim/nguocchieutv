		...
		<div class="<?php echo esc_attr(trim(implode(' ', $parent_container_classes))); ?>">
			...
			if ( $posts_counter > 0 ):
				...
				foreach ( $posts as $key_post => $post ):
					...
					$typeLayout = $is_tax_query ? $post_blocks['uncode_taxonomy'] : $post_blocks['uncode_' . str_replace('-', '_', $post->type)];
					$single_layout = $is_tax_query ? 'single_layout_uncode_taxonomy_items' : 'single_layout_' . $post->type . '_items';
