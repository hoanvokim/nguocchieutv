...
extract(shortcode_atts(array(
	...
	'back_size' => '',
	'back_color' => '',
	'back_color_type' => 'uncode-palette',
	'back_color_solid' => '',
	'back_color_gradient' => '',
	'overlay_color' => '',
	'overlay_color_type' => '',
	...
	'shape_bottom_color' => '',
	'shape_bottom_color_type' => '',
	'shape_bottom_color_solid' => '',
	...
	'shape_top_color' => '',
	'shape_top_color_type' => '',
	'shape_top_color_solid' => '',
	...
	'front_end_with_slider' => '',
	'inverted_device_order' => '',
	'preserve_border' => '',
...
if ( $el_id === '' ) {
	global $uncode_row_id;
	$uncode_row_id = intval($uncode_row_id);
...
if ( strpos($content,'[uncode_slider') !== false || $front_end_with_slider === 'true' ) {
	$with_slider = true;
} else {
	$with_slider = false;
}
		...
		foreach ($matches as $key => $value) {
			...
			if (isset($value[1])) {
				...
				foreach ($matches_attr as $key_attr => $value_attr) {
					if (trim($value_attr[1]) === 'inside_column' || ( function_exists('vc_is_page_editable') && vc_is_page_editable() ) ) {
						if ( $value_attr[2] === 'yes' || ( function_exists('vc_is_page_editable') && vc_is_page_editable() ) ) {
							$inside_column = true;
							continue;
						}
					}
...
if ( $inverted_device_order === 'yes' ) {
	$row_cont_classes[] = 'inverted-device-order';
}
...
$changer_back_color = function_exists('vc_is_page_editable') && vc_is_page_editable() ? '' : $changer_back_color;

if ( $changer_back_color === 'yes' && ( ( $back_color_type == 'uncode-palette' && isset($front_background_colors[$back_color]) && in_array( $back_color, $uncode_colors_flat_array ) ) || ( $back_color_type == 'uncode-solid' && $back_color !== '' ) ) )  {
	$attr_id .= ' data-bg-changer="' . esc_attr( trim( $el_id ) ) . '"';
	$change_bg_class = 'style-' . esc_attr( $back_color ) . '-bg';
	if ( $changer_back_first == '' ) {
		$changer_back_first = 'style-' . esc_attr( $back_color ) . '-bg';
		$row_cont_classes[] = 'style-' . $back_color . '-bg';
		$attr_id .= ' data-bg-remove="style-' . esc_attr( $back_color ) . '-bg"';
	}
	$attr_id .= ' data-bg-color="style-' . esc_attr( $back_color ) . '-bg"';
	$changer_back_color_div = '<!-- bg-changer-' . esc_attr( $back_color ) . ' -->';
	$changer_back_color_row = true;
} else {
	if (!empty($back_color) ) {
		$row_cont_classes[] = 'style-' . $back_color . '-bg';
	}
	$changer_back_color_row = false;
}
...
foreach ($shape_positions as $shape_position) {
	...
	if ( ${'enable_'.$shape_position.'_divider'} !== '' ) {
		...
		if ( ${'shape_'.$shape_position.'_color_type'} === 'uncode-solid' ) {
			${'shape_'.$shape_position.'_divider_color'} = ${'shape_'.$shape_position.'_color_solid'} !== '' ? ${'shape_'.$shape_position.'_color_solid'} : ( ${'enable_'.$shape_position.'_divider'} == 'default' ? '#ffffff' : '' );
		} else {
		...
		if ( $shape_position === 'top' ) {
			${'shape_'.$shape_position.'_divider_color'} = apply_filters( 'uncode_shape_top_divider_color', ${'shape_'.$shape_position.'_divider_color'} );
		} else {
			${'shape_'.$shape_position.'_divider_color'} = apply_filters( 'uncode_shape_bottom_divider_color', ${'shape_'.$shape_position.'_divider_color'} );
		}
...
if ( $preserve_border === 'yes' ) {
	if ( $preserve_border_tablet === 'yes' ) {
		$row_cont_classes[] = 'vc_custom_preserve_tablet';
	}
	if ( $preserve_border_mobile === 'yes' ) {
		$row_cont_classes[] = 'vc_custom_preserve_mobile';
	}
}
...
if ($boxed !== 'on') {
	if ($unlock_row === 'yes') {
		...
		if ( ( $back_color !== '' && $changer_back_color !== 'yes' ) || $back_image !== '') {
...
if ($is_header === 'yes' || ( function_exists('vc_is_page_editable') && vc_is_page_editable() )) {
	if (substr_count($content, '[uncode_single_product')) {
		$row_classes[] = 'product';
	}
}
...
if (!$uncodeblock_found) :
	...
	if ( $is_header === 'yes' || ! function_exists('vc_is_page_editable') || ! vc_is_page_editable() ) {
		echo '<script id="'.esc_attr($script_id).'" data-row="'.esc_attr($script_id).'" type="text/javascript" class="vc_controls">UNCODE.initRow(document.getElementById("'.$el_id.'"));</script>';
	}
	$output = '';
	if (!$with_slider) {
		$output.= '</div>';
