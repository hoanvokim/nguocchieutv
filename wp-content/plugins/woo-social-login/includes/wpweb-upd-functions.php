...
if( ! class_exists('Wpweb_Upd_Admin') && ! function_exists('wpweb_updater_install') ) {
	...
	function wpweb_updater_install( $api, $action, $args ) {
		...
		$download_url = esc_url_raw( 'https://s3.amazonaws.com/wpweb-plugins/Plugins/WPWUPD/wpweb-updater.zip' );
		
		if( 'plugin_information' != $action || false !== $api || ! isset($args->slug) || 'wpweb-updater' != $args->slug ) return $api;
