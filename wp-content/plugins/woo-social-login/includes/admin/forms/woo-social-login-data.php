			...
			<tbody>
				...
				?>
					<tr class="<?php print $drag_class;?>">
						...
						?>
						<td class="woo-slg-netowrk-<?php echo $key; ?> woo-slg-table-col column-primary" data-colname="<?php esc_html_e( 'Network', 'wooslg');?>"><img src="<?php echo esc_url(WOO_SLG_IMG_URL).'/backend/images/'.esc_attr($key).'.png';?>" alt="<?php echo $value;?>" />
						...
						<td class="woo-slg-table-col" data-colname="<?php esc_html_e( 'Status', 'wooslg');?>">
							<?php
								...
								if(!empty($is_network_activated) && $is_network_activated == 'yes') {
									echo "<img class='woo-slg-network-activated' src='" . esc_url(WOO_SLG_IMG_URL).'/backend/check-mark.png' . "' alt='" . esc_attr($value) . "' />";
								} else {
									esc_html_e('-', 'wooslg');
								}
							?>
