...
class Uncode_Toolkit_Privacy_Public {
	...
	public function set_plugin_cookies() {
		...
		if ( ! isset( $_COOKIE['uncode_privacy']['consent_types'] ) ) {
			if ( ! $user_id ) {
				setcookie( 'uncode_privacy[consent_types]', '[]', $this->get_cookies_conf() );
			} else {
				$user_consents = get_user_meta( $user_id, 'uncode_privacy_consents' );

				setcookie( "uncode_privacy[consent_types]", json_encode( $user_consents ), $this->get_cookies_conf() );
			}
		} else {
			if ( $user_id ) {
	...
	public function update_privacy_preferences() {
		...
		setcookie( "uncode_privacy[consent_types]", $consents_as_json, $this->get_cookies_conf() );
