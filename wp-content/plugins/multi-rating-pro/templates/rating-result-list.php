...
<div class="rating-results-list <?php if (isset( $class ) ) { echo esc_attr( $class ); } ?>">
	...
	if ( ! empty( $title ) ) {
		...
		$before_title = apply_filters( 'mrp_rating_results_list_before_title', $before_title );
		$after_title = apply_filters( 'mrp_rating_results_list_after_title', $after_title );
	...
	if ( $show_filter == true && $taxonomy ) {
		...
		<form action="" class="mrp-filter" method="POST">
			...
			<div class="wp-block-button">
				<input class="wp-block-button__link" type="submit" value="<?php echo esc_attr( $filter_button_text ); ?>" />
			</div>
		</form>
		<?php
	
		do_action( 'mrp_rating_results_list_after_filter' );
	}
	...
	if ( count( $rating_results ) == 0 ) {
		...
		foreach ( $rating_results as $rating_result ) {
			...
			do_action( 'mrp_rating_results_list_row_before_first_td', $post_id, $rating_result );
			...
			if ( $show_featured_img == true ) {
				if ( has_post_thumbnail( $post_id ) ) {
					...
					<td class=" mrp-featured-img">
						<?php
						do_action( 'mrp_rating_results_list_before_featured_img', $post_id );
						echo get_the_post_thumbnail( $post_id, $image_size );
						do_action( 'mrp_rating_results_list_after_featured_img', $post_id );
						?>
					</td>
					<?php
			...
			do_action( 'mrp_rating_results_list_row_after_last_td', $post_id, $rating_result );
