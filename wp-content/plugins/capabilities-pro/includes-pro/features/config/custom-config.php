...
class EditorFeaturesCustomConfig {
    ...
    public function customAddFormClassic() {
        ...
        <tr class="ppc-add-custom-row-body">
            <td colspan="3">
                ...
                <div class="left">
                <?php echo __('Label', 'capsman-enhanced'); ?> <font color="red">*</font>
                <input class="ppc-feature-classic-new-name" type="text"/>
                <small><?php echo __('Enter the name/label to identify the custom element on this screen.',
                ...
                <div>
                    <small><?php echo __('IDs or classes to hide. Separate multiple values by comma (.custom-item-one, .custom-item-two, #new-item-id).',
                            'capsman-enhanced'); ?></small>
                </div>
                </div>
    ...
    public static function addByAjax()
    {
        ...
        if (!wp_verify_nonce($security, 'ppc-custom-feature-nonce')) {
            ...
            $custom_element = sanitize_textarea_field($custom_element);
