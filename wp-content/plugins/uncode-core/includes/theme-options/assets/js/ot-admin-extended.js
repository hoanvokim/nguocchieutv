	...
	OT_UI.uncode_init = function() {
		...
		OT_UI.adaptive = function() {
			$(document).ready(function() {
				// Make this check only on the theme options page
				if ($('#option-tree-settings-api').length > 0) {
					$.ajax({
						...
						complete: function(response) {
							...
							$('#ai-space-wrap').append('<div class="ai-space">' + response.responseJSON.data.message + '</div>');
							var button = $('<button class="option-tree-ui-button button">Delete all images</button>');
							button.on('click', function(e) {
								...
								$.ajax({
									...
									complete: function(response) {
										...
										$('#ai-space-wrap .ai-space').html(response.responseJSON.data.message);
									}
								});
								return false;
							});
							$('#ai-space-wrap').append(button);
						}
					});
				}
			});
		...
		OT_UI.init_select_wrapper = function() {
			var is_msie = (window.navigator.userAgent.indexOf('MSIE') !== -1 || window.navigator.userAgent.indexOf('Trident/') !== -1 || window.navigator.userAgent.indexOf('Edge/') !== -1 );
			...
			$(document).on(is_msie ? 'click' : 'change', '.option-tree-ui-select', function(event) {
...
! function($) {
	$(document).ready(function() {
		...
		$('.button_icon_container').each(function() {
			$(this).loadIcons();
		});
